	...
	*new {|interpret|
	...
	synthesis {
		...
		Server.default.waitForBoot({
			interpreter.compileFile(basePath ++ "/Controller.scd").value;
	...
	observe {|ndef, amp=0.7|
		var name = ("" ++ ndef).asSymbol; name.postln;
		this.report("observe", name);
		Ndef(name, { PlayBuf.ar(2, buffers[\gestfinevolve]) * amp * XLine.ar(0.00001, 1.0, 22) }).play;
		^Ndef(name);
	}
	...
	dust {|type=0, time=5, rate=1, amp=0.8, durFrom=0.1, durTo=0.9, modFrom=0.1, modTo=8|
		...
		Pdef(name, Pbind(\instrument, \dustrace, \rate, rate, \buf, buf, \pos, Pif(Ptime(inf) < time,
			...
			\amp, Prand([1.4,1.2,1.6,0,0,0],inf) * amp, \dur, Pwhite(durFrom,durTo))).play;
